...
pub trait Handler {
	...
	fn on_message(&mut self, message: ChatMessageEventPacket) -> HandlerResult;
	
	/// Fired when a user joins the channel.
	fn user_join(&mut self, user: User) -> HandlerResult { HandlerResult::Nothing }
	/// Fired when a user leaves the channel.
	fn user_leave(&mut self, user: User) -> HandlerResult { HandlerResult::Nothing }
	/// Fired when a user is timed out in chat.
	fn on_user_timeout(&mut self, user: &str, length: u16) -> HandlerResult { HandlerResult::Nothing }
	/// Fired when a user is purged in chat.
	fn on_user_purged(&mut self, user: &str) -> HandlerResult { HandlerResult::Nothing }
	...
	fn on_message_deleted(&mut self, message: &str) -> HandlerResult { HandlerResult::Nothing }
}
